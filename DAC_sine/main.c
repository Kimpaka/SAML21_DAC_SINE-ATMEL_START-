...
{
	uint16_t buf = 0;
	if(_dac_sync_is_channel_enable(&DAC_0.device, 1))	{
		if (wave == SINE_WAVE)	{
			buf = sine_wave[sample_number];
//			buf = 0x200;
//			dac_sync_write(&DAC_0, 1, &buf, 1);
			_dac_sync_write_data(&DAC_0.device, buf, 1);
		}
		else	{
			buf = triangle_wave[sample_number];
		}
...
int main(void)
{
	...
	dac_sync_enable_channel(&DAC_0, 1);
